...
def should_parse(
    ...
    window and merged. A ``pages`` or ``max_pages`` filter parses so only
    those 1-based pages are sent to the model.
    """
    return (
        cite
        or pages is not None
        or max_pages is not None
        or style is ExtractionStyle.TABLE
        or style is ExtractionStyle.FORM
    )
