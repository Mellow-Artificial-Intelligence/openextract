...
| Many inputs on a reusable session | Session `extract_many` / `extract_many_with_results` (same names on `AsyncExtractor`) |
...
| Per-field source spans (ExtractBench grounding) | `cite=True` on `extract_with_result*` / `extract_many_with_results*` or sessions (`Extractor` / `AsyncExtractor`). Read `ExtractionResult.citations`; `extract()` / session `extract()` still return the schema instance. Swarm: `extract_swarm_with_results*` → `SwarmResult.citations` (per-agent cites stay on each agent). `Citation.confidence` / `match` are local heuristics (not model-provided). `cite_min_confidence` drops weak or unstamped cites after grounding. `ExtractionResult.warnings` records that drop (and page-filter drops) as short count strings; happy path is `()`. Review helpers: `field_confidence`, `citations_by_field`, `filter_citations` (also on `ExtractionResult`). |
...
| Model timeout / settings | `timeout=30` and `model_settings={...}` on `extract*` / batch / swarm / sessions. `timeout` overrides a `timeout` entry in `model_settings`. Invalid `timeout` raises `ValueError` before any model call. CLI: `--timeout SECONDS`. Default off (provider default). |
