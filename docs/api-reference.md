...
`url_timeout` is the HTTP timeout for fetching `http(s)` URL inputs, matching
session `url_timeout`. `None` uses `OPENEXTRACT_URL_TIMEOUT` or 30 seconds;
invalid values raise `ValueError` before any fetch or model call.
...
| `url_timeout` | `float \| None` | HTTP timeout in seconds for fetching `http(s)` URL inputs. Same contract as session `url_timeout`. `None` (default) uses `OPENEXTRACT_URL_TIMEOUT` or 30 seconds. Must be a finite number greater than 0; invalid values raise `ValueError` before any fetch or model call. |
...
`model_settings`. Oneshot, batch, swarm, and session `url_timeout` controls
URL input fetching (`None` uses `OPENEXTRACT_URL_TIMEOUT` or 30 seconds).
