...
invoice = extract(Invoice, "openai:gpt-5", "invoice.pdf")
print(invoice.total)
```
...
- **`source`** is a file path, a `pathlib.Path`, an `http(s)://` URL, or raw `bytes`.
...
```python
extract(Invoice, "anthropic:claude-sonnet-5", "https://example.com/invoice.pdf")
extract(Invoice, "openai:gpt-5", scanned_page_bytes, "Amounts are in EUR.")
```
